...
function initializeGlobalFeatures() {
    Object.getOwnPropertyNames(_extensionMap, function (extBasename) {
        if (extBasename.global) {
            var featureObj = {
                id: _extensionMap[extBasename].namespace,
                version: "1.0.0.0",
                required: true
            };
            _globalFeatures.push(featureObj);
        }
    });
}
...
function initialize(session) {
    ...
    if (path.existsSync(extPath)) {
        ...
        extDirs.forEach(function (dir) {
            var manifestPath = path.normalize(path.resolve(path.join(dir, MANIFEST_FILE))),
                manifest,
                basename;
            ...
            if (path.existsSync(manifestPath)) {
                try {
                    ...
                    basename = manifestPath.split(path.sep).pop(); // get extension base name
